...
struct HashTable *hash_table_init(size_t capacity, size_t key_len, size_t val_len)
{
	...
	ht->val_len = val_len;
	ht->size = 0;
	unsigned slot_len = 1 + key_len + val_len;   // 1 = status --> 0, 1 or 2
	ht->data = malloc(capacity * slot_len);
	ht->capacity = ht->data ? capacity : 0;    // if ht->data != 0 -> ht->capacity = capacity ;; else -> ht->capacity = 0
 	for (unsigned i = 0; i < ht->capacity; i++) {
		unsigned char *p = (unsigned char *)ht->data + i * slot_len;
		p[0] = FREE_SLOT;   //mark every slot as free
	}
	return ht;
}
 ...
 */
uint32_t hash_key(void *key, unsigned key_len) //hash function
{
...
void *hash_table_find(const struct HashTable *ht, void *key)
{
	...
	for (size_t probe = 0; probe < ht->capacity; probe++) {  
		...
		if (p[0] == FREE_SLOT) {  //if its a free slot: means that key should have been put
			return NULL;
		}
		if ((p[0] != DELETED_SLOT) &&
		    memcmp(key, p + 1, ht->key_len) == 0) {     //compare given number of bytes of data pointed by two pointers
				//p+1 to access key 
			return p + 1 + ht->key_len;  //-> val
		} else {
			pos++;
		}
	}
...
void hash_table_insert(struct HashTable *ht, void *key, void *val)
{
	...
	/* Hash table is full */
	printf("Hash table is full ! Implement hash_table_grow or initialize with a larger capacity.\n");
	abort();
}
...
void hash_table_delete(/*const*/ struct HashTable *ht, void *key){
	...
	void *val = hash_table_find(ht, key); //points to val, function returns void
	if(!val){
		return;  //void 
	}
	unsigned char *p = val - ht->key_len - 1;  //go back to beginning of the slot 
	ht->size--;   //size = nb of occupied slots --> if we do this, we should not put the "const"
	p[0] = DELETED_SLOT;


	/*  first idea... should work as well?? but way too long....
	uint32_t pos = hash_key(key, ht->key_len);
	unsigned slot_len = 1 + ht->key_len + ht->val_len;
	unsigned char *data = ht->data;
	...
	for (size_t probe = 0; probe < ht->capacity; probe++) {
		...
		//if key is present, then mark as deleted and decrease size
		if (hash_table_find(ht, key) == data + pos * slot_len + 1 + ht->key_len) {
			ht->size--;
			p[0] = DELETED_SLOT;
			return NULL;
		} else {
