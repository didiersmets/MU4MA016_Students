...
double area_mesh2D(struct Mesh2D* m) {
    ...
    for(int i = 0; i < trinum; i++) {
        struct Vector ab = vect(m->vertices[m->triangles[i].a],m->vertices[m->triangles[i].b]);
        struct Vector ac = vect(m->vertices[m->triangles[i].a],m->vertices[m->triangles[i].c]);
        double tri_area = (ab.x_length*ac.x_length + ab.y_length*ac.y_length)/2;
        area += tri_area;
    }
    return area;
}
...
int read_mesh2D(struct Mesh2D* m, const char* filename) {
    ...
    sscanf(line,"MeshVersionFormatted %d",&version);//fscanf(fp,"MeshVersionFormatted %d\n Dimension %d Vertices %d",&version, &dimension, &num_vertices);
    fgets(line, sizeof(line), fp);
    sscanf(line,"Dimension %d",&dimension);
    fgets(line, sizeof(line), fp);
    sscanf(line,"Vertices %d",&num_vertices);
    m->nvert = num_vertices;
    m->vertices = (Vertex*) malloc(2*sizeof(double)*num_vertices);
    for(int i = 0; i < num_vertices; i++){
        double current_coordinate = 0;
        fscanf(fp,"%lf",&current_coordinate);
    ...
    int num_triangles = 0;
    fscanf(fp,"\nTriangles %d",&num_triangles);
    m->ntri = num_triangles;
    printf("numtriangles read as: %d \n",num_triangles);
    ...
    for(int i = 0; i < num_triangles; i++){
        ...
        int cc = 0;
        double refe = 0;
        fscanf(fp,"%d %d %d %lf",&aa,&bb,&cc,&refe);
        m->triangles[i].a = aa;
        m->triangles[i].b = bb;
        m->triangles[i].c = cc;
        }
...
int write_mesh2D(struct Mesh2D* m, const char* filename) {
    ...
    for(int i = 0; i < num_vertices; i++){
        ...
        fprintf(fp,"%lf ",current_coordinate);
